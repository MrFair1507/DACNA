...
exports.getTasksByBacklog = async (req, res) => {
  ...
  FROM Tasks t
  LEFT JOIN Task_Assignment a ON t.task_id = a.task_id
  LEFT JOIN Users u ON a.user_id = u.user_id
  WHERE t.sprint_backlog_id = ?
  `,
...
exports.createTask = async (req, res) => {
  const { task_title, task_description, priority, start_date, due_date } =
    req.body;
  ...
  try {
    ...
    if (!task_title || !task_status || !priority) {
      return res.status(400).json({
        error: "Thiếu trường bắt buộc (task_title, task_status, priority)",
      });
    }
    ...
    res
      .status(500)
      .json({ error: "Cập nhật task thất bại", detail: err.message });
  }
};
...
exports.deleteTask = async (req, res) => {
  try {
    await db.execute(`DELETE FROM Tasks WHERE task_id = ?`, [
      req.params.task_id,
    ]);

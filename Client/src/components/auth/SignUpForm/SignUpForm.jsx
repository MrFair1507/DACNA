...
//           // If there's a specific message from the server, display it
//           if (result.message) {
//             setSuccessMessage(result.message);
//           } else {
//             setSuccessMessage("OTP đã được gửi tới email của bạn.");
//           }
//         } else {
//           setErrors({ general: result.error || "Lỗi đăng ký không xác định" });
//         }
//       } catch (error) {
...
const SignUpForm = () => {
  ...
  const navigate = useNavigate();
  const { register, verifyOTP, oauthLogin, resendOTP } = useAuth();
